...
slack.init((data, ws) => {
    ...
    slack.getGroups((error, response, userData) => {
        ...
        groups = parsedGroupData.groups.filter(group => !group.is_archived);
...
components.groupsList.on('select', (data) => {
    ...
    // get user's id
    const groupId = groups.find(potentialUser => potentialUser.name === groupname).id;

    slack.openGroup(groupId, (error, response, imData) => {
        const parsedImData = JSON.parse(imData);
        currentChannelId = groupId; // Same method as in find openUser does not work
        // load im history
        slack.getGroupHistory(currentChannelId, (histError, histResponse, groupHistoryData) => {
